...
class generic_class extends db_model implements iModel{
	...
	public function get($id = 0){
		...
		*
		* Si el valor del parametro Id es diferente a 0, se solicitara solo y unicamente
		* el elemento cuyo Id sea igual al parametro recibido.
		*/
		if ($id == 0) {
			return $this->get_query(sprintf("SELECT * FROM %s", $this->entity));
		// Si el valor del parametro Id es diferente a 0, se solicitara solo y unicamente el elemento cuyo Id
		// sea igual al parametro recibido
		}
		else {
			return $this->get_query(sprintf("SELECT	* FROM %s WHERE Id = %d", $this->entity, $id));
		}
	}

	// Esta funcion sera llamada al momento de usar el metodo POST
	public function post(){
		return $this->set_query(sprintf("INSERT INTO %s %s", $this->entity, $this->data));}

	// Esta funcion sera llamada al momento de usar el metodo PUT
	public function put(){
		return $this->set_query(sprintf("UPDATE %s SET %s WHERE Id = %d", $this->entity, $this->data, $this->Id));
	}

   // Esta funcion sera llamada al momento de usar el metodo DELETE
	public function delete(){
	 	return $this->set_query(sprintf("DELETE FROM %s WHERE Id = %d",$this->entity, $this->Id));
	}
}
?>